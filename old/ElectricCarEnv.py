...
class ElectricCarEnv(gym.Env):
    ...
    def __init__(self) -> None:
        ...
        self.observation_space = spaces.Box(low=np.array([0, 0, 0, 0]), high=np.array([self.max_battery, 24, 1, 5000]), dtype=np.float32)
    ...
    def step(self, action: float) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        # Update the state
        self.state = np.array([self.battery_level, self.time_of_day, self.car_available, price])
